...
void CS_Init(void){
  ...
  GPIO_PORTD_DR4R_R |= 0x84;            // 4mA drive on outputs
...
void SSI2_Init(uint32_t CPSDVSR){
  ...
  GPIO_PORTB_DR4R_R |= 0xF2;            // 4mA drive on outputs (still need to add PD2)
  GPIO_PORTB_PUR_R |= 0xF0;             // enable weak pullup on PB 4, 5, 6, 7
  GPIO_PORTB_DEN_R |= 0xF0;             // enable digital I/O on PB 4, 5, 6, 7
                                        // configure PB4, 6, 7
